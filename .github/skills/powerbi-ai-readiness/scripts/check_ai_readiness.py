...
`scripts/check_ai_readiness.py` at a host repo's root may be a forwarding shim; the real file ships
inside the `powerbi-ai-readiness` skill. See that skill's SKILL.md for what "AI-ready" means.
"""
...
def main() -> None:
    ...
    if args.all:
        ...
        targets = sorted(
            p for tree in MIGRATION_TREES for p in (REPO_ROOT / tree).glob("*") if (p / "fabric").is_dir()
        )
    elif args.migration:
