...
- [**`scripts/probe_desktop_query.py`**](scripts/probe_desktop_query.py) - read-only preflight: port
  ...
  imports from this file and sibling bundled helpers, never a host repository's allocator.
- [**`scripts/_operator_pause.py`**](scripts/_operator_pause.py) - explicit retained-popup context,
  protected publication, strict exact-pause loader and mechanical cleanup. No continuation policy.
- [**`scripts/probe_desktop_credential.ps1`**](scripts/probe_desktop_credential.ps1) - the arbiter a
  ...
  (exit 1) is the credential-specific hard stop**, because only it has positive authentication-specific
  evidence; every other observation lands in the exit-3 band (`REFRESH_IN_PROGRESS` /
  `DIALOG_NEEDS_HUMAN` / `DIALOG_UNRECOGNIZED` / `DIALOG_UNREADABLE` / `UNKNOWN`). Exit 3 is *not*
  "nobody is needed": it means non-clean, and its tokens ask for a wait, an inspection, or the action
  the visible prompt names — see the verdict table below.
...
python scripts/refresh_pbip_model.py [--pid <pbidesktop-pid>] [--canaries "A" "B"] [--tables "A" "B"]
                                     ...
                                     [--ui-save]
                                     [--evidence-dir <scratch> | --operator-pause-scratch <run\scratch>]
```
...
There is no retained-mode environment variable, working-directory fallback, path-shape inference,
latest-run selection or image adoption/copy.

Before starting the operation the caller resolves its exact PID-bound model and calls
`prepare_operator_pause(run_scratch, model_dir, desktop_pid)`. The Python API accepts the resulting
immutable `OperatorPauseContext` as `refresh(..., operator_pause=context)`. The selected scratch
must be an existing ordinary local directory named exactly `scratch`; its parent's ordinary
`run.json` must agree on `run`, `unit_key`, `allocated_dir_name` and absolute `allocated_abs_path`.
The model must be below that same run, outside scratch/packages/deliverables. UNC, device,
reparse/symlink, moved, artifact/package/deliverable ancestry and nonignored checkout locations
refuse. Repository-local and short-root allocations use the same contract; no `work_dirs` import
or allocator discovery is needed by a copied-out skill.

**Eligibility is not broadened:** only the existing **in-flight `DIALOG_UNREADABLE`** observation
can queue acquisition. A t=0 finding behaves exactly as before, with no capture. Readable,
unrecognized, progress, credential and native-query observations do not enter this retained path.
No OCR, source text, prompt category or requested action is inferred. The existing one-attempt
PID/HWND limit, pending-slot backpressure, original eight-second acquisition budget and original
refresh deadline still apply. Storage and acquisition run on the observer, never the refresh wait.

The same acquisition child writes **directly** into a protected owner-only ordinary `prompt.png`
in a private `.pending-<pause-id>-<nonce>` directory. All existing PID/HWND/direct-owner,
owner-disabled, DPI/physical-frame, dimensions, PNG and hash checks remain; exact Desktop
process-creation FILETIME is additionally checked around capture and again before publication.
The opaque pause ID is 32 lowercase hexadecimal characters, generated by the producer, never a
caller-supplied path fragment.

Publication flushes/fsyncs protected CreateNew `pause.json`, rechecks identity/containment and
the deadline/closed state, closes the image handles, and renames the staging directory to its
same-parent final name **without replacement**. It then commits a protected, ordinary zero-byte
`READY` **last**, still within that original deadline. An exclusively held provisional READY
lease is delete-on-close until committed, so slow storage or forced producer exit at that boundary
cannot expose an accepted incomplete marker. **The retained image has no expiry or delete-on-close
lease.** There is no sudden-power-loss durability claim.

```text
<selected-run>\scratch\operator-pauses\<pause-id>\
    prompt.png
    pause.json
    READY
```
...
Captured image: <absolute selected-run scratch path to prompt.png>
Pause ID: <opaque id>
```

The human notice is **not a new refresh verdict**. A refresh already in flight can still finish.
Only that local human layer contains an absolute path; the machine record and `pause.json` contain
relative locators and the following **closed** `pbip.operator-pause.v1` fields:

| Field | Exact meaning |
|---|---|
| `schema`, `pause_id`, `state`, `phase`, `reason` | `pbip.operator-pause.v1`, opaque ID, `waiting`, `pbip_refresh`, `DIALOG_UNREADABLE` |
| `captured_at_utc`, `published_at_utc` | Millisecond UTC timestamps ending in `Z`, capture no later than publication |
| `run` | Integer `number` and allocated `directory` basename |
| `model_path` | Model locator relative to the selected run, `/`-separated |
| `desktop` | Decimal-string `pid`, `process_start` (creation FILETIME), `dialog_hwnd`, `owner_hwnd` |
| `image` | Exact `path: prompt.png`, lowercase `sha256`, numeric `dimensions.width/height`, and Boolean `ownership_checks.before/after_render/after_write`, all true |

No absolute host paths, prompt/OCR text, SQL, visible values, credentials, source keys, canaries,
spec/bundle hashes, AS identity, classification, confirmation or result-policy fields are accepted.
The image itself is private local evidence: do not copy it into logs, Git, packages or deliverables.
PID creation identity closes cross-process PID reuse, not an undetectable HWND destruction/reuse
entirely between snapshots within one process.

**Exact consumption and cleanup:** require the notice's exact ID and expected hash and call
`load_operator_pause(run_scratch, pause_id, expected_hash)`. It validates canonical run/model
binding, paths/reparse state, protected ACLs, complete closed schema, zero-byte READY, PNG
dimensions and hash; a missing/malformed/unready pause is `OPERATOR_PAUSE_CANNOT_ESTABLISH`.
Open **that** image in an ordinary local viewer; retained evidence needs no shared-delete reader.
Never enumerate pauses to select newest.

`cleanup_operator_pause(run_scratch, pause_id, expected_hash)` repeats those checks and returns
`removed`, `absent`, `cannot_establish` or `cleanup_failed`. It claims all three exact files before
deleting any; a holding ordinary viewer leaves the pause intact. It never deletes a sibling,
parent, glob or guessed/latest pause. A repeated call cannot affect another ID; **`absent` is only
mechanical absence**, never evidence of operator confirmation or a cleared gate.

A timely successful refresh requests cleanup of only its own retained pauses on the evidence
observer. Timeout/dialog results, failed XMLA workers, session exit and forced producer exit retain
fully READY evidence. Cleanup never holds the refresh wait or rewrites its result:
`OPERATOR_PAUSE_CLEANUP` reports the exact ID/hash and mechanical status, and an unfinished success
cleanup at normal producer exit reports `cleanup_failed` rather than silently claiming removal.
Close a holding viewer and use exact mechanical cleanup when needed. Unready crash remnants are
not accepted pauses and are not selected or swept automatically.

**Non-goals:** no automatic Continue, confirmation/result schemas, operator statements, gate
transition, probe streaming, status/dispatcher/package changes, source query, native-query
approval, DAX/cache corroboration or cold reopen. Those belong to later policy slices. This helper
and its tests travel with this skill; the root forwarding shim is unchanged.
...
> | `CREDENTIAL_PRESENT` | 0 | a refresh was invoked and **no** credential prompt was detected before this probe's own deadline. A bounded observation, not proof of a cached credential, of a running refresh, or of a live Desktop — a process that dies mid-poll enumerates zero windows and lands here. Never the gate of record: confirm with the one-row data probe. |
...
> | `DIALOG_NEEDS_HUMAN` | 3 | a **known** human-blocking prompt matched — the **native-database-query approval modal**, an approval request, or an `Authentication required` notice. Not exit 1, because the credential signature did not match; but the remedy is **not** established either, since that one signature spans an approval *and* an authentication notice. Act on the prompt visible in Desktop. Never suppressed, and it outranks any progress text in the same window. |
> | `DIALOG_UNRECOGNIZED` | 3 | a dialog is up whose text matched **no** signature, **or** which shows progress text *alongside* prose that is not progress status. We read it and could not account for it, and the signature list is not exhaustive — so the credential state is **undetermined in both directions**. A human must look at the screen. |
> | `DIALOG_UNREADABLE` | 3 | a dialog is up that could not be **shown to be harmless**: no text at all, or only a reassuring **caption**, or benign-looking content read from an **incomplete** harvest (truncated, timed out, or a pattern that threw). Its content was never established, so the credential state is **undetermined in both directions**. Deliberately distinct from `DIALOG_UNRECOGNIZED`: *absent is not empty*, and "we could not establish it" is a weaker state of knowledge than "we read it and it did not match". |
...
>   `REFRESH_IN_PROGRESS` to `DIALOG_UNRECOGNIZED` — both exit 3. Only the progress token concludes
>   anything ("a refresh already owns this pid; wait or cancel"); `DIALOG_UNRECOGNIZED` concludes
>   nothing about the credential state in either direction. Keep its
...
>
> 1. **`scripts/blocking_prompt_signature.regex`** — known human-blocking prompts (native-query
>    approval, `Authentication required`). Matched **before** benign and **before** the enabled-owner
>    exoneration, and reported as **`DIALOG_NEEDS_HUMAN`, exit 3** — a human must act, but the token
>    does not establish *which* action, because that one signature spans an approval **and** an
>    authentication notice. It stays out of the exit-1 band whose documented meaning is *"sign in
>    once"*, and the human reads the prompt on screen.
...
>    signature was, at this round, a word count: a content element of **5+ words** that is not itself
>    recognised status vetoed suppression, while short data labels (`Orders`, `1,204 rows loaded`,
>    `Cancel`) did not. ⚠️ **That word count is history** — the length-amnesty finding below (PR #400
>    blind review, finding 5) replaced it with an enumerated chrome allowlist, because length is not
>    evidence.
...
> ✅ **Length amnesty, current state: the arbiter has none, and issue #406 is closed.**
> `probe_desktop_credential.ps1` once carried the same five-word amnesty the Python detector did — the
> measured shape was `Refresh` + `Evaluating` + `Please enter your password` → `benign` →
> `REFRESH_IN_PROGRESS`, and suppressed to `$null` under `-RefreshInFlight`; `Password:` likewise. It
> was filed rather than fixed in passing (the precedent that created #376), and it has since been
> removed: short unknown text now vetoes suppression, and the **enumerated** chrome allowlist is the
> only exemption. Do not re-add a size or length test on either side.
>
> **Reachability, stated rather than hidden.** Removing the amnesty costs the detector its
