...
|---|---|
| `read_handover.py <bundle> --workbook <name> --viz [--severity X]` | **your work queue**: `remediation_worklist` (per item `severity`, `category`, `reason`, `remediation`), **emptied** visuals — every binding dropped, so they render blank on a report that validates clean (15 in one measured workbook, previously unremarked; ⚠️ at 2.353.0 the reader is still the only thing that ranks these — upstream #189 adds `pbip_ref_drops[].severity: blocking` only at 2.355.0) — and `viz_fidelity[]` (`status`, `tier`, `reason`). Reading the raw 347 KB slice by hand works but buries these; see `powerbi-report-gotchas` §10 |
...
1. **Assert the model is WARM before you open Desktop — and never self-refresh.** The semantic builder
   ...
   **Read the baseline before changing anything.** Open the rebuilt report and screenshot every page
   ...
   because it is per-visual. (⚠️ Narrowed upstream: #188 adds `page_emitted:false` at the three
   drop sites from 2.354.0 — above our 2.353.0, and it never claims `true`, so a *missing* page is
