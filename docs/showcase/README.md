...
> Generated by `scripts/make_showcase.py` from `docs/showcase/showcase.json`. Run with `--layout stacked` for tall, LinkedIn/mobile-friendly versions, or `--order after-before` to lead with the Power BI result. See [`README-afterbefore.md`](README-afterbefore.md) for the after/before variant.
