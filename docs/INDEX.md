...
1. `python scripts/check_reference_readiness.py <bundle>` — **after deterministic emission, before agentic fidelity-building/review dispatch**; see the authoritative [conversion, dispatch, and fidelity boundaries](reference-readiness.md#conversion-dispatch-and-fidelity-boundaries), including the pending final package consumer. Per page it reports completeness, evidence and **grade**. On a self-contained handover package it first proves the boundary, then that `package-manifest.json` still describes the bytes on disk, then that every role the unit's kind and topology require is present with agreeing identities (#562) — pass the provider and consumer packages of a shared datasource in ONE invocation, because a consumer cannot prove its provider alone. Exit 0 ready / 1 findings / 3 `CANNOT_ESTABLISH`, and neither 1 nor 3 is a pass — a blind page means an equivalent fidelity bug there is **structurally unfalsifiable**, not merely unverified. `NOT_APPLICABLE` (a datasource-only unit) is earned from the engine's own `report.json`, so it never blocks legitimately reference-free work.
