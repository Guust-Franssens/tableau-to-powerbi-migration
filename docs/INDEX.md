...
| Validate an artifact | [`scripts/check_ai_readiness.py`](../scripts/check_ai_readiness.py) | Run or inspect when model descriptions, CustomInstructions, domains, or qnaEnabled are in scope. |
| Validate an artifact | [`scripts/check_blank_placeholders.py`](../scripts/check_blank_placeholders.py) | Run or inspect when PBIR text boxes, titles, labels, or generated placeholders may be blank. |
| Validate an artifact | [`scripts/check_datamodel.py`](../scripts/check_datamodel.py) | Run or inspect before trusting M/TMDL structure or model deserialization. |
| Validate an artifact | [`scripts/check_empty_model.py`](../scripts/check_empty_model.py) | Run or inspect when Desktop opens a model/report empty or a cache/model handoff looks suspect. |
| Validate an artifact | [`scripts/check_engine_receipts.py`](../scripts/check_engine_receipts.py) | Run or inspect when validating deterministic-engine provenance and canonical engine receipts. |
| Validate an artifact | [`scripts/check_field_bindings.py`](../scripts/check_field_bindings.py) | Run or inspect when visuals may bind missing columns/measures or incorrect projections. |
...
| Validate an artifact | [`scripts/check_pbir_layout.py`](../scripts/check_pbir_layout.py) | Run or inspect when PBIR page geometry, visual bounds, or off-canvas layout may be invalid. |
| Validate an artifact | [`scripts/check_pbir_valid.py`](../scripts/check_pbir_valid.py) | Run or inspect when PBIR schema validation and report-author CLI results must be enforced. |
| Validate an artifact | [`scripts/check_relationship_health.py`](../scripts/check_relationship_health.py) | Run or inspect when model relationships, cardinality, active paths, or ambiguity are in scope. |
