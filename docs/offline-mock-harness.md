...
| [`tests/mutation_harness.py`](../tests/mutation_harness.py) | 36 deliberate mutations that prove those tests can fail — **22** attack this offline-mock suite (tabled below); the other **14** attack the skill-plugin sync (`skillsync-` / `skillsource-`, issue #410), which shares this harness rather than growing a second scorer |
...
> The harness is **shared**, not offline-mock-specific: it also carries 14 mutations against the
