...
- A dialog is up that is *not* a credential prompt -> one of `REFRESH_IN_PROGRESS` (another refresh
  ...
  alongside content that is neither recognised progress status nor enumerated chrome — an unaccounted
  **caption** counts) or `DIALOG_UNREADABLE` (its **content** could not be
