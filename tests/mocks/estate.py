...
:func:`install_fake_engine`
    ...
    **This is a stand-in, not a mock of measured behaviour.** The real engine is a ~200-file plugin
    ...
    + ``<name>.Report`` with a ``byPath`` dataset reference. It parses each workbook with THIS repo's
    real parser, so the count of pages it emits is derived from the workbook, not hard-coded.
"""
...
def emit(spec, out, name):
    ...
    pages = [w.get("name") for w in (spec.get("worksheets") or [])]
    report = project / (name + ".Report")
    (report / "definition" / "pages").mkdir(parents=True, exist_ok=True)
    (report / "definition.pbir").write_text(
        json.dumps({{"version": "4.0", "datasetReference": {{"byPath": {{"path": "../" + name + ".SemanticModel"}}}}}}),
        encoding="utf-8",
    )
    (report / "definition" / "pages" / "pages.json").write_text(
        json.dumps({{"pageOrder": pages or [], "activePageName": pages[0] if pages else ""}}), encoding="utf-8"
    )
