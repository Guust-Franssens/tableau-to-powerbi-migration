...
@dataclass
class Estate:
    """A bare `origin`, a clone of it, and an installed plugin root to publish into."""
...
def _estate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Estate:
    ...
    return Estate(origin=origin, clone=clone, plugin=plugin)
...
def test_the_generator_also_comes_from_the_ref(estate: Estate, capsys: pytest.CaptureFixture) -> None:
    ...
    (estate.clone / ".github" / "skills" / "branch-only-bundle").mkdir(parents=True)
    (estate.clone / ".github" / "skills" / "branch-only-bundle" / "SKILL.md").write_text("nope\n", encoding="utf-8")
