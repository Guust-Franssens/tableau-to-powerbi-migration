...
| #194 | [`issue-194-long-pbir-path`](issue-194-long-pbir-path/README.md) | Reproduces **at 2.368.0**: `_MAX_FS_BASE = 64` caps the emitted *folder* bases but not `<model>.SemanticModel/definition/tables/<table>.tmdl`, so an ordinary long export filename pushes a **required** child to **273** UTF-16 units at the skill's own `C:\tfmig\runs\NNNN\out` root. Engine exit 0; Power BI Desktop refuses with *"Issues were found … too long"* naming that exact path, while the short control opens and renders real data. A downloadable `.twbx` A/B, not a pin. |
| #424 | [`issue-424-automatic-mark-discrete-date`](issue-424-automatic-mark-discrete-date/README.md) | Reproduces **at 2.339.0** (upstream [#184](https://github.com/Yarbrdab000/tableau-fabric-skills/issues/184)): an `Automatic` mark over a *discrete* date emits a stacked `columnChart` where Tableau draws a line — silently (`tier: rebuilt`, no warning, empty worklist). **8 fixtures, not 3**: four defect pins plus four permanent invariants that bound how a fix may be written. |
