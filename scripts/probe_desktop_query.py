...
purpose: forwarding shim. The real script now ships INSIDE the `pbip-model-refresh` skill, at
         `.github/skills/pbip-model-refresh/scripts/probe_desktop_query.py`, so the skill is one
         self-contained copyable unit. This keeps `python scripts/probe_desktop_query.py ...`
...
TARGET = REPO_ROOT / ".github" / "skills" / "pbip-model-refresh" / "scripts" / "probe_desktop_query.py"


def main() -> int:
    """Run the bundled script in this process, with `sys.argv` untouched."""
    if not TARGET.exists():
        print(f"PREFLIGHT: ERROR bundled script not found at {TARGET}")
        return 2
