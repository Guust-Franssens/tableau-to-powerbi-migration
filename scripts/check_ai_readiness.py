...
purpose: forwarding shim. The real script now ships INSIDE the `powerbi-ai-readiness` skill, at
         `.github/skills/powerbi-ai-readiness/scripts/check_ai_readiness.py`, so the skill is one
         self-contained copyable unit. This keeps `python scripts/check_ai_readiness.py ...`
         working - `.github/agents/pbi-semantic-builder.agent.md` and `docs/` still invoke that path.
usage:   python scripts/check_ai_readiness.py <tree>/<slug> [--strict]
         python scripts/check_ai_readiness.py --all
...
TARGET = REPO_ROOT / ".github" / "skills" / "powerbi-ai-readiness" / "scripts" / "check_ai_readiness.py"


def main() -> int:
    """Run the bundled script in this process, with `sys.argv` untouched."""
    if not TARGET.exists():
        print(f"AI-READINESS: ERROR bundled script not found at {TARGET}")
        return 2
