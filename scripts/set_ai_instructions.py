...
def iter_models(root: Path) -> list[Path]:
    ...
    `examples/` holds this repo's worked examples; `migrations/` and `datasources/` hold the user's own
    workbook and published-data-source migrations. All three share the <tree>/<slug>/fabric/ shape.
    """
    return sorted(
        p
...
def cmd_check(root: Path) -> int:
    ...
    if not models:
        log.info("No models found under %s (examples/, migrations/, datasources/)", root)
        return 0
    missing = 0
    for model in models:
        try:
