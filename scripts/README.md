...
| `gate_matrix.py` | Runs the credential gate end-to-end across many models on the happy or unhappy path, with a per-run budget and kill-on-deviation so a misbehaving model costs minutes, not hours. Serial by design (concurrent Desktop instances cross-contaminate). **Judges from artifacts and the audit log, never the agent's self-report** — and refuses to score a run that crashed or never engaged, because "gate armed AND zero artifacts" is something a dead process satisfies for free. Results table: [`docs/credential-gate-testing.md`](../docs/credential-gate-testing.md) | manually, when the gate or a persona changes |
| `provision_snowflake_fixture.py` | Creates the Snowflake test fixture (warehouse, database, schema, 400-row `SHIPMENT`) mirroring the Databricks one, so both connector paths are tested identically. Key-pair auth, **infrastructure only** — never a Power BI credential, which is the distinction the whole gate rests on. | manually, once per test account |
| `setup_snowflake_keypair.py` | One-time Snowflake key-pair setup: generates the RSA pair into gitignored `secrets/`, registers the public key on the user, and verifies it authenticates. Preferred over a PAT because key-pair auth needs **no network policy** (a PAT is refused with `390432` until one is *attached* to the user) and survives the workstation's IP rotating. | manually, once per machine |
| `snowflake_fixture.sql` | The same fixture as hand-runnable SQL for Snowsight, plus the network-policy SQL that Snowflake PAT auth requires. Use this when the PAT is refused with `390432` (a policy must be **attached** to the user, not merely created) — it needs no PAT at all. | manually, in Snowsight |
