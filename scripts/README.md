...
| `setup_snowflake_keypair.py` | One-time Snowflake key-pair setup: generates the RSA pair into gitignored `secrets/`, prints the one `ALTER USER` that registers the public key, and verifies it authenticates. Preferred over a PAT because key-pair auth needs **no network policy** (a PAT is refused with `390432` until one is *attached* to the user) and survives the workstation's IP rotating. | manually, once per machine |
| `snowflake_fixture.sql` | The same fixture as hand-runnable SQL for Snowsight, for when you would rather not run the Python at all. Also records the network-policy SQL a PAT would need — kept as the reason we *don't* use one. | manually, in Snowsight |
