...
usage:   python scripts/work_dirs.py <unit-name> [--repo-root PATH] [--json]
         python scripts/work_dirs.py --select-run ABSOLUTE-EXISTING-RUN
...
_NAVIGATION_NOTE = "_MIGRATION.md"
_NAVIGATION_MARKER = "<!-- Generated by scripts/work_dirs.py; replaceable migration navigation. -->"
...
def _selection_path_problem(value: str) -> str | None:
    """Reject non-local or traversal-dependent spellings before touching the selected path."""
...
# pylint: disable-next=too-many-return-statements
def _admit_selected_run(run_dir: Path) -> str | None:
    """No-follow admission for one explicit local run root, before any selected-run read."""
    value = os.fspath(run_dir)
    problem = _selection_path_problem(value)
    if problem is not None:
        return problem

    anchor = Path(run_dir.anchor)
    chain = [anchor]
    current = anchor
    ...
    for entry in chain:
        try:
            ...
            return f"cannot inspect local path component {entry} ({exc.__class__.__name__}: {exc})"
        if is_reparse_entry(info):
            return f"local path component is a symlink, junction or reparse point: {entry}"
        if not stat.S_ISDIR(info.st_mode):
            return f"local path component is not a directory: {entry}"

    manifest_path = run_dir / "run.json"
    try:
        manifest_info = os.lstat(manifest_path)
    except OSError as exc:
        return f"cannot inspect run.json ({exc.__class__.__name__}: {exc})"
    if is_reparse_entry(manifest_info) or not stat.S_ISREG(manifest_info.st_mode):
        return "run.json is not a regular non-reparse file"
    return None


def _load_selected_run(run_dir: Path) -> tuple[RunPaths | None, str | None]:
    """Admit and validate one explicitly selected run without inferring or scanning."""
    problem = _admit_selected_run(run_dir)
    if problem is not None:
        return None, problem

    manifest, status, detail = _read_run_manifest(run_dir)
    if status != MANIFEST_OK or manifest is None:
        return None, detail
    location = check_run_location(manifest, run_dir)
    if location.state != RUN_LOCATION_INTACT:
        return None, location.detail
    return (
        RunPaths(root=run_dir, run_number=manifest["run"], unit_key=manifest["unit_key"]),
        None,
    )


def _navigation_note_is_private() -> tuple[bool, str]:
    """Establish that the fixed checkout note is ignored and untracked before writing paths."""
    try:
        ignored = subprocess.run(
            ["git", "-C", str(REPO_ROOT), "check-ignore", "--quiet", "--", _NAVIGATION_NOTE],
            check=False,
            capture_output=True,
        )
        ...
        return False, f"local Git privacy checks could not run ({exc.__class__.__name__}: {exc})"
    if ignored.returncode != 0:
        return False, f"{_NAVIGATION_NOTE} is not ignored by this toolkit checkout"
    if tracked.returncode != 1:
        return False, f"{_NAVIGATION_NOTE} is tracked or its tracked state could not be established"
    return True, ""


def _render_navigation_note(run: RunPaths) -> str:
    """Render fixed navigation labels from known paths only; inspect no generated artifacts."""
    toolkit_root = Path(os.path.abspath(str(REPO_ROOT)))
    run_root = Path(os.path.abspath(str(run.root)))
    try:
        common = os.path.commonpath((str(toolkit_root), str(run_root)))
        inside = os.path.normcase(common) == os.path.normcase(str(toolkit_root))
    except ValueError:
        inside = False
    relation = "inside toolkit" if inside else "outside toolkit"
    generated = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
    return (
        f"{_NAVIGATION_MARKER}\n"
...
# pylint: disable-next=too-many-return-statements
def _write_navigation_note(run: RunPaths) -> tuple[bool, str]:
    """Create or replace only this generator's ignored, untracked checkout note."""
    private, detail = _navigation_note_is_private()
    if not private:
        return False, detail

    target = REPO_ROOT / _NAVIGATION_NOTE
    try:
        info = os.lstat(target)
    except FileNotFoundError:
        try:
            with target.open("x", encoding="utf-8", newline="\n") as handle:
                handle.write(_render_navigation_note(run))
        except OSError as exc:
            return False, f"could not exclusively create {target} ({exc.__class__.__name__}: {exc})"
        return True, ""
    except OSError as exc:
        return False, f"could not inspect {target} ({exc.__class__.__name__}: {exc})"

    if is_reparse_entry(info) or not stat.S_ISREG(info.st_mode):
        return False, f"{target} is not a regular non-reparse file"
    try:
        existing = target.read_text(encoding="utf-8")
    except (OSError, ValueError) as exc:
        return False, f"could not read existing {target} ({exc.__class__.__name__}: {exc})"
    if existing.splitlines()[:1] != [_NAVIGATION_MARKER]:
        return False, f"{target} is not marked as generator-owned"
    try:
        target.write_text(_render_navigation_note(run), encoding="utf-8", newline="\n")
    except OSError as exc:
        return False, f"could not update {target} ({exc.__class__.__name__}: {exc})"
    return True, ""


def _select_run_for_navigation(run_dir: Path) -> tuple[bool, str]:
    """Validate one explicit selection, then update only the checkout navigation note."""
    selected, problem = _load_selected_run(run_dir)
    if selected is None:
        return False, f"selected run was refused: {problem}"
    return _write_navigation_note(selected)


def main() -> int:
    """CLI: allocate a run, select an existing run for navigation, or `--verify` the tree."""
    ...
    action_group.add_argument(
        ...
        type=Path,
        default=None,
        metavar="ABSOLUTE-EXISTING-RUN",
        help=(
            "allocate nothing; validate one explicitly accepted existing local run and update the "
            "toolkit checkout's generated navigation note"
        ),
    )
    args = parser.parse_args()

    if args.select_run is not None:
        if args.unit or args.repo_root is not None or args.json:
