...
ROOT = r"C:\Users\gfranssens\vscode-projects\tableau-to-pbi-migration\migrations\airline-alliance-activity"
...
DTYPE = {"integer": ("int64", "Int64.Type"), "real": ("double", "type number"),
         "string": ("string", "type text"), "date": ("dateTime", "type date")}
...
# ================================================================ MEASURE DEFINITIONS
REGION = ("FILTER(ALL('{F}'[Origin Region]),[Region Parameter Value]=\"Global\"||'{F}'[Origin Region]=[Region Parameter Value])").format(F=FACT)
REGION_DEST = ("FILTER(ALL('{F}'[Destination Region]),[Region Parameter Value]=\"Global\"||'{F}'[Destination Region]=[Region Parameter Value])").format(F=FACT)
def col(c): return "'%s'[%s]" % (FACT, c)
...
def AGG(kind, c=None):
    if kind == "distinct": return "DISTINCTCOUNT('%s'[Flight Id])" % FACT
    if kind == "sum":      return "SUM(%s)" % col(c)
    if kind == "avg":      return "AVERAGE(%s)" % col(c)
    if kind == "sumx_pd":  return "SUMX('%s',%s*%s)" % (FACT, col("Passengers Carried"), col("Distance Km"))
    if kind == "sumx_cd":  return "SUMX('%s',%s*%s)/1000" % (FACT, col("Cargo Carried Kg"), col("Distance Km"))
    if kind == "sum1000":  return "SUM(%s)/1000" % col(c)
    raise ValueError(kind)

CUR="\\$#,##0"; INT="#,##0"; PCT="0.0%"; DEC2="0.00"
REG = {
...
BASE_SET = {
 "CY": ["All Flights","Avg. CSAT","CTK","Cargo Load Factor","Costs","Count On-Time Perf","Freight Tonnes","Load Factor","No of Comp Flights","Op Profit","Passenger Count","RPK","RPK (A)","Total Revenue"],
 "PY": ["Avg. CSAT","CTK","Cargo Load Factor","Count On-Time Perf","Freight Tonnes","Load Factor","No of Comp Flights","Op Profit","Passenger Count","RPK","RPK (A)","Total Revenue"],
 "CM": ["All Flights","Avg. CSAT","CTK","Cargo Load Factor","Costs","Count On-Time Perf","Freight Tonnes","Load Factor","No of Comp Flights","No of Mishandled Baggage","Op Profit","Passenger Count","RPK","RPK (A)","Total Revenue"],
 "PM": ["All Flights","Avg. CSAT","CTK","Cargo Load Factor","Costs","Count On-Time Perf","Freight Tonnes","Load Factor","No of Comp Flights","No of Mishandled Baggage","No of Passengers","Op Profit","Passenger Count","RPK","RPK (A)","Total Revenue"],
}
...
def base_dax(period, metric):
    ...
    for e in extra: preds.append(EXTRA[e])
    if airline: preds.append(EXTRA["airline"])
    preds.append(REGION)
...
for period in ["CY","PY","CM","PM"]:
    for metric in BASE_SET[period]:
        ...
        add_m(nm, dax, fmt, FOLDER[period],
              False,
...
# ---- ratio measures
add_m("CY On-Time Perf%", "DIVIDE([CY Count On-Time Perf],[CY All Flights])", PCT, "Ratios", False,
      "Tableau 'CY On-Time Perf%': CY on-time flights / CY all flights.")
add_m("CM On-Time Perf%", "DIVIDE([CM Count On-Time Perf],[CM All Flights])", PCT, "Ratios", False,
      "Tableau 'CM On-Time Perf%': CM on-time flights / CM all flights.")
add_m("PM On-Time Perf %", "DIVIDE([PM Count On-Time Perf],[PM All Flights])", PCT, "Ratios", False,
      "Tableau 'PM On-Time Perf %': PM on-time flights / PM all flights.")
add_m("CM % of Mishandled Baggage", "DIVIDE([CM No of Mishandled Baggage],[CM No of Comp Flights])", PCT, "Ratios", False,
      "Tableau 'CM % of Mishandled Baggage': CM mishandled-baggage flights / CM completed flights.")
add_m("PM % of Mishandled Baggage", "DIVIDE([PM No of Mishandled Baggage],[CM No of Comp Flights])", PCT, "Ratios", False,
...
for nm, cy, py in CIRCLE:
    ...
    note = ""
    if nm == "Costs Circle Col": note = " NOTE: source formula compares CY/PY Op Profit (not Costs) - faithful reproduction of a source quirk."
    if nm == "On-Time Perf Circle Col": note = " Compares raw CY/PY on-time flight COUNT (not the %)."
    add_m(nm, dax, None, "Trend Indicators (YoY)", False,
...
for nm, cm, pm in MOM:
    ...
    add_m(nm, dax, PCT, "MoM Growth (Positive)", False,
          "Tableau '%s': month-over-month growth (%s/%s - 1); BLANK when not growing." % (nm, cm, pm))

# ---- period helper measures (on fact)
add_m("PM Month Value", "IF([Month Parameter Value]=1,12,[Month Parameter Value]-1)", INT, "Parameters", True,
      "Prior-month month number derived from the Month parameter (wraps 1->12).")
add_m("PM Year Value", "IF([Month Parameter Value]=1,[Year Parameter Value]-1,[Year Parameter Value])", INT, "Parameters", True,
      "Prior-month year derived from the Year/Month parameters (rolls back at January).")
add_m("Date From Params Value", "DATE([Year Parameter Value],[Month Parameter Value],1)", "Short Date", "Parameters", True,
...
add_m("Selected Region", "[Region Parameter Value]", None, "Parameters", False, "Tableau 'Selected Region' = current Origin Region parameter value.")
add_m("Selected Airline", "[Airline Parameter Value]", None, "Parameters", False, "Tableau 'Selected Airline' = current Airline Name parameter value.")
add_m("Selected Aircraft Type", "[Aircraft Parameter Value]", None, "Parameters", False, "Tableau 'Selected Aircraft Type' = current Aircraft Type parameter value.")

# ---- colour / layout helper measures
add_m("Colour", 'IF([Month Parameter Value]=SELECTEDVALUE(\'Date\'[Month Number]),"Month","Others")', None, "Report Helpers", False,
      "Tableau 'Colour': highlights the month matching the Month parameter (assumes visual grouped by Date[Month Number]).")
add_m("Year Colour", 'IF([Year Parameter Value]=SELECTEDVALUE(\'Date\'[Year]),"Year","Others")', None, "Report Helpers", False,
      "Tableau 'Year Colour': highlights the year matching the Year parameter (assumes visual grouped by Date[Year]).")
add_m("Bar Colours", 'IF(SELECTEDVALUE(\'Date\'[Month Start])=[Date From Params Value],"Selected Month","Others")', None, "Report Helpers", False,
      "Tableau 'Bar Colours': highlights the selected month bar (assumes visual grouped by Date[Month Start]).")
add_m("Airline Bar Colours", 'IF(SELECTEDVALUE(\'Date\'[Month Start])=[Date From Params Value]&&SELECTEDVALUE(\'%s\'[Airline Name])=[Airline Parameter Value],"Selected Month","Others")' % FACT, None, "Report Helpers", False,
...
add_m("CM Comp Flights to Destination", dax, INT, "Current Month (CM)", False,
...
def measure_block(m):
    ...
    if fmt: out.append("%s%sformatString: %s" % (T, T, fmt))
    if folder: out.append("%s%sdisplayFolder: %s" % (T, T, folder))
    if hidden: out.append("%s%sisHidden" % (T, T))
    out.append("")
    return "\n".join(out)

def column_block(caption, source, dtype, fmt=None, hidden=False, sortby=None, calc=None, desc=None):
    out = []
    if desc: out.append("%s/// %s" % (T, esc(desc)))
    if calc:
    ...
    if sortby: out.append("%s%ssortByColumn: '%s'" % (T, T, sortby))
    if hidden: out.append("%s%sisHidden" % (T, T))
    out.append("")
...
part.append('%s%s%s%sSource = Csv.Document(File.Contents(DataFolder & "%s"),[Delimiter=",", Columns=70, Encoding=65001, QuoteStyle=QuoteStyle.Csv]),' % (T,T,T,T, CSVFILE))
part.append('%s%s%s%s#"Promoted Headers" = Table.PromoteHeaders(Source, [PromoteAllScalars=true]),' % (T,T,T,T))
part.append('%s%s%s%s#"Changed Type" = Table.TransformColumnTypes(#"Promoted Headers",{%s}, "en-US")' % (T,T,T,T, mtypes))
...
for caption, source, dtype, mtype, dt in cols:
    fmt = None
    if dtype == "int64": fmt = "#,##0"
    if dtype == "dateTime": fmt = "Short Date"
    fact_lines.append(column_block(caption, source, dtype, fmt=fmt))
# Aircraft Name calc column
fact_lines.append(column_block("Aircraft Name", None, "string",
...
def param_table(table_name, col_name, dtype, rows, default, value_measure, folder="Parameters", ordered=True, extra_desc=""):
    ...
    lines = []
    lines.append("/// Disconnected single-select slicer table backing the Tableau '%s' parameter (default %s).%s Native-slicer + SELECTEDVALUE equivalent of a Tableau live parameter (parameter-equality idiom)." % (table_name, (q % default), (" " + extra_desc if extra_desc else "")))
    lines.append("table '%s'" % table_name)
    lines.append("")
    dv = ('"%s"' % default) if is_str else str(default)
    lines.append("%s/// Reads the current '%s' slicer selection, defaulting to the Tableau default %s." % (T, table_name, dv))
    ...
    if not is_str: lines.append("%s%sformatString: #,##0" % (T, T))
    lines.append("%s%ssummarizeBy: none" % (T, T))
    lines.append("%s%ssourceColumn: %s" % (T, T, col_name))
    if ordered: lines.append("%s%ssortByColumn: '%s Order'" % (T, T, col_name))
    lines.append("")
...
w(os.path.join(TBL, "Year Parameter.tmdl"), param_table(
    "Year Parameter", "Year", "integer",
    [(2022,0),(2023,1),(2024,2),(2025,3)], 2023, "Year Parameter Value", ordered=True))
w(os.path.join(TBL, "Month Parameter.tmdl"), param_table(
    "Month Parameter", "Month", "integer",
    [(m,m) for m in range(1,13)], 7, "Month Parameter Value", ordered=False,
    extra_desc="Month number 1-12."))
w(os.path.join(TBL, "Region Parameter.tmdl"), param_table(
    "Region Parameter", "Region", "string",
    [("Global",0),("Africa",1),("Asia",2),("Europe",3),("Middle East",4),("North America",5),("Oceania",6),("South America",7)],
    "North America", "Region Parameter Value", ordered=True,
    extra_desc="'Global' is the show-all sentinel (Origin Region filter is bypassed)."))
w(os.path.join(TBL, "Airline Parameter.tmdl"), param_table(
    "Airline Parameter", "Airline", "string",
    [("Atlantic Express",0),("Global Star",1),("Pacific Wings",2),("SkyConnect Airways",3)],
    "SkyConnect Airways", "Airline Parameter Value", ordered=True))
w(os.path.join(TBL, "Aircraft Type Parameter.tmdl"), param_table(
    "Aircraft Type Parameter", "Aircraft Type", "string",
    [("A320",0),("A330",1),("A350",2),("B737",3),("B777",4),("B787",5)],
    "A330", "Aircraft Parameter Value", ordered=True))
w(os.path.join(TBL, "Airline Code Parameter.tmdl"), param_table(
    "Airline Code Parameter", "Airline Code", "string",
    [("AE",0),("GS",1),("PW",2),("SC",3)],
    "GS", "Airline Code Parameter Value", ordered=True,
    extra_desc="Report-level slicer; not consumed by any KPI formula (parity only)."))
w(os.path.join(TBL, "Guidelines Parameter.tmdl"), param_table(
    "Guidelines Parameter", "Guideline", "string",
    [("Show Guidelines",0),("Hide Guidelines",1)],
    "Hide Guidelines", "Guidelines Value", ordered=True,
...
tables_order = ["Flight Activity","Date","Year Parameter","Month Parameter","Region Parameter",
                "Airline Parameter","Aircraft Type Parameter","Airline Code Parameter","Guidelines Parameter"]
...
expr = ('expression DataFolder = "%s" meta [IsParameterQuery=true, Type="Text", IsParameterQueryRequired=true]\n\n\tannotation PBI_ResultType = Text\n' % DATADIR)
