...
SPEC = r"C:\Users\gfranssens\vscode-projects\tableau-to-pbi-migration\migrations\electricity-per-capita\migration-spec.json"
...
DISP = {
    ...
    ("tree", "X fixed for last path"): (CC, "Tree", "DIMENSION-FIXED LOD {FIXED [id]: MAX(IF type='link' THEN path)} -> CALCULATE(MAX(Path), ALLEXCEPT(Tree,Id), Type='link'). Ground truth mlp verified."),
    ("tree", "Y fixed for last path"): (CC, "Tree", "FIXED[id] LOD on Y (internal 'X doubled for last path (copy)')."),
    ("tree", "Calculation1"): (CC, "Tree", "FIXED[id] LOD {FIXED [id]: AVG(Usage)} -> CALCULATE(AVERAGE(Usage), ALLEXCEPT(Tree,Id))."),
    ...
    ("tree", "CY Consumption"): (ME, "Elec Generation", "DIMENSION-FIXED LOD {FIXED [child]: sum(IF Year=CY THEN percap)}; CY hoisted to VAR. Ground truth Norway@2023=28056.230."),
    ("tree", "Calculation2"): (ME, "Elec Generation", "Boolean [CY Consumption] > {FIXED [Region]: AVG(percap)}; measure hoisted to VAR (no compact filter)."),
    ("tree", "Max year"): (ME, "Elec Generation", "WINDOW table calc WINDOW_MAX(MAX([Year]))=MAX([Year]) -> CALCULATE(MAX(Year), ALLEXCEPT(Entity)) vs MAX(Year) in context. Norway@2023=28056.230, @2022=BLANK."),
    ("tree", "Elec gen at 2022"): (CC, "Elec Generation", "IF [Year]=2022 THEN percap. Relocated row-level column."),
    ("tree", "Max year (copy)"): (CC, "Elec Generation", "IF [Year]=[CY] THEN percap -> calc column 'Consumption at CY'."),
    ("tree", "Region and country count"): (CC, "Elec Generation", "FIXED[Region] COUNTD(Entity) -> ALLEXCEPT(Region) label string."),
    ...
    ("tree", "X-trellis"): (DR, "-", "Small-multiple grid X = (index()-1)%int(SQRT(size())). Native PBI small-multiples feature."),
    ...
    # ---- Pivoted ----
    ("pivoted", "FF Electricity"): (CC, "Pivoted", "DIMENSION-FIXED LOD {FIXED [Year],[Code]: SUM(FF)} -> CALCULATE(SUM, FILTER(ALLEXCEPT(Year,Code), Fuel='Fossil Fuel')). USA 2022=7559.257."),
    ...
    ("pivoted", "Region and country count"): (CC, "Pivoted", "FIXED[Region] COUNTD(Country) -> ALLEXCEPT(Region) label."),
    ...
    # ---- Elec Generation ----
    ("elecgen", "Global avg in 2022"): (ME, "Elec Generation", "Literal 3616.7 (=FNR World 2022 total). Constant reference-line measure."),
...
lines.append(f"Every one of the **{len(allrows)}** Tableau calculated fields across the 5 data sources is dispositioned below.\n")
lines.append(f"**Totals:** {fate_counts[CC]} calc columns, {fate_counts[ME]} measures, {fate_counts[DR]} dropped, {fate_counts[GAP]} capability gaps.\n")
lines.append("- **calc column** / **measure** - translated into the model (table shown).")
lines.append("- **dropped** - visual-layout table calc (trellis/index/window-normalization) or redundant alias; belongs to the report, not the model.")
lines.append("- **capability gap** - spatial MAKEPOINT geometry for the Sankey; no native Power BI equivalent (see limitations_encountered).\n")
...
order = ["ds.per_capita_electricity_fossil_nuclear_renewables", "ds.tree",
         "ds.pivoted_per_capita_electricity_generation_by_source",
         "ds.elec_generation_per_capita_regions", "ds.by_source"]
for ds_id in order:
    lines.append(f"\n## {DS_TITLE[ds_id]}\n")
...
with open(OUT, "w", encoding="utf-8") as f:
    f.write("\n".join(lines) + "\n")
print("Wrote", OUT)
print(f"Coverage: {len(allrows)} calc fields | calc-col={fate_counts[CC]} measure={fate_counts[ME]} dropped={fate_counts[DR]} gap={fate_counts[GAP]}")
