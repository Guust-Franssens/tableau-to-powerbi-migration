...
MTYPE = {"string": "type text", "int64": "Int64.Type", "double": "type number", "dateTime": "type datetime"}
...
TABLE_SPECS.append(
    dict(
        name="Per Capita FNR",
        csv="ds.per_capita_electricity_fossil_nuclear_renewables.csv",
        ...
        calc_cols=[
            (
                "FF%",
                f"DIVIDE({FNR}[Fossil fuel electricity per capita - kWh], {FNR}[Fossil fuel electricity per capita - kWh] + {FNR}[Nuclear electricity per capita - kWh] + {FNR}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                "Nuclear%",
                f"DIVIDE({FNR}[Nuclear electricity per capita - kWh], {FNR}[Fossil fuel electricity per capita - kWh] + {FNR}[Nuclear electricity per capita - kWh] + {FNR}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                "Renewables %",
                f"DIVIDE({FNR}[Renewable electricity per capita - kWh], {FNR}[Fossil fuel electricity per capita - kWh] + {FNR}[Nuclear electricity per capita - kWh] + {FNR}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
...
TABLE_SPECS.append(
    dict(
        name="Tree",
        csv="ds.tree.Extract.tree.csv_D5C7546A97E64FBE8DB0BCDADDF65312.csv",
        ...
        calc_cols=[
            ...
            (
                "X revised (for nodes)",
                f'IF({T}[Type] = "node" && NOT ISBLANK({T}[Usage]) && {T}[Path] = 0, DIVIDE({T}[Usage], MAXX(ALL({T}), {T}[Usage])) * {T}[X Normalized])',
                "double",
                False,
                "none",
            ...
            (
                "Y revised (for nodes)",
                f'IF({T}[Type] = "node" && NOT ISBLANK({T}[Usage]) && {T}[Path] = 0, DIVIDE({T}[Usage], MAXX(ALL({T}), {T}[Usage])) * {T}[Y Normalized])',
                "double",
                False,
                "none",
            ...
            (
                "X fixed for last path",
                f'VAR mlp = CALCULATE(MAX({T}[Path]), ALLEXCEPT({T}, {T}[Id]), {T}[Type] = "link") RETURN IF({T}[Path] = mlp, ({T}[Usage ration] * {T}[X Normalized]) + {T}[X Normalized], {T}[X Normalized])',
                "double",
                False,
                "none",
            ...
            (
                "Y fixed for last path",
                f'VAR mlp = CALCULATE(MAX({T}[Path]), ALLEXCEPT({T}, {T}[Id]), {T}[Type] = "link") RETURN IF({T}[Path] = mlp, ({T}[Usage ration] * {T}[Y Normalized]) + {T}[Y Normalized], {T}[Y Normalized])',
                "double",
                False,
                "none",
            ...
            (
                "Calculation1",
                f'VAR a = CALCULATE(AVERAGE({T}[Usage]), ALLEXCEPT({T}, {T}[Id])) RETURN IF({T}[Type] = "node" && ISBLANK({T}[X revised (for nodes)]), a)',
                "double",
                False,
                "none",
            ...
            (
                "Filter",
                f'{T}[Type] = "link" && ISBLANK({T}[Parent])',
                "string",
                False,
                "none",
        ...
        measures=[
            ...
            ("X Axis Rank", "0", "0.0", "Axis anchors", "Tableau [X Axis - Rank] = MIN(0.0): constant 0 axis anchor."),
            ("X Axis Text", "0", "0.0", "Axis anchors", "Tableau [X Axis - Text] = MIN(0.0): constant 0 axis anchor."),
        ],
    )
)
...
TABLE_SPECS.append(
    dict(
        name="Pivoted",
        csv="ds.pivoted_per_capita_electricity_generation_by_source.csv",
        ...
        calc_cols=[
            ...
            (
                "FF Electricity%",
                f"DIVIDE({P}[FF Electricity], {P}[FF Electricity] + {P}[Nuclear Electricity] + {P}[Renewable Electricity])",
                "double",
                False,
                "none",
            ...
            (
                "Nuclear Electricity%",
                f"DIVIDE({P}[Nuclear Electricity], {P}[FF Electricity] + {P}[Nuclear Electricity] + {P}[Renewable Electricity])",
                "double",
                False,
                "none",
            ...
            (
                "Renewable Electricity%",
                f"DIVIDE({P}[Renewable Electricity], {P}[FF Electricity] + {P}[Nuclear Electricity] + {P}[Renewable Electricity])",
                "double",
                False,
                "none",
            ...
            (
                "Region and country count",
                f'{P}[Region] & UNICHAR(10) & UNICHAR(10) & CALCULATE(DISTINCTCOUNT({P}[Country]), ALLEXCEPT({P}, {P}[Region])) & " countries"',
                "string",
                False,
                "none",
...
TABLE_SPECS.append(
    dict(
        name="Elec Generation",
        csv="ds.elec_generation_per_capita_regions.Extract.Elec generation per capita _ regions.csv_B3B07BC3AEC34A68859236D28C71EB13.csv",
        ...
        calc_cols=[
            (
                "Elec gen at 2022",
                f"IF({E}[Year] = 2022, {E}[Per capita electricity - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                "Consumption at CY",
                f"VAR cy = CALCULATE(MAX({E}[Year]), ALL({E})) RETURN IF({E}[Year] = cy, {E}[Per capita electricity - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                "Region and country count",
                f'{E}[Region] & UNICHAR(10) & UNICHAR(10) & CALCULATE(DISTINCTCOUNT({E}[Entity]), ALLEXCEPT({E}, {E}[Region])) & " countries"',
                "string",
                False,
                "none",
        ...
        measures=[
            ...
            (
                "CY Consumption",
                f"VAR cy = CALCULATE(MAX({E}[Year]), ALL({E})) RETURN CALCULATE(SUM({E}[Per capita electricity - kWh]), {E}[Year] = cy)",
                "#,##0.000",
                "Consumption",
            ...
            (
                "Max year",
                f"VAR emax = CALCULATE(MAX({E}[Year]), ALLEXCEPT({E}, {E}[Entity])) RETURN IF(MAX({E}[Year]) = emax, SUM({E}[Per capita electricity - kWh]))",
                "#,##0.000",
                "Consumption",
            ...
            (
                "Calculation2",
                f"VAR regavg = CALCULATE(AVERAGE({E}[Per capita electricity - kWh]), ALLEXCEPT({E}, {E}[Region])) RETURN [CY Consumption] > regavg",
                None,
                "Consumption",
            ...
            (
                "Ratio",
                f"DIVIDE(AVERAGE({E}[Per capita electricity - kWh]), [Global avg in 2022])",
                "0.00",
                "Consumption",
    ...
    dict(
        ...
        calc_cols=[
            (
                ...
                f"DIVIDE({B}[Fossil fuel electricity per capita - kWh], {B}[Fossil fuel electricity per capita - kWh] + {B}[Nuclear electricity per capita - kWh] + {B}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                ...
                f"DIVIDE({B}[Nuclear electricity per capita - kWh], {B}[Fossil fuel electricity per capita - kWh] + {B}[Nuclear electricity per capita - kWh] + {B}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
            ...
            (
                ...
                f"DIVIDE({B}[Renewable electricity per capita - kWh], {B}[Fossil fuel electricity per capita - kWh] + {B}[Nuclear electricity per capita - kWh] + {B}[Renewable electricity per capita - kWh])",
                "double",
                False,
                "sum",
...
PARAM_SPECS = [
    dict(
        ...
        col_type="string",
        col_fmt=None,
        values=["Tree", "Table", "Table no region"],
        default_dax='"Tree"',
        vfmt=None,
        ...
        extra_measures=[
            (
                "DZV show tree map",
                'SELECTEDVALUE(\'Show chart as\'[Show chart as], "Tree") = "Tree"',
                None,
                "View toggles",
            ...
            (
                "DZV table with region",
                'SELECTEDVALUE(\'Show chart as\'[Show chart as], "Tree") = "Table"',
                None,
                "View toggles",
                "Tableau [DZV: table with region] = [Parameters].[Parameter 1]='Table'.",
            ),
            (
                "DZV table without region",
                'SELECTEDVALUE(\'Show chart as\'[Show chart as], "Tree") = "Table no region"',
                None,
                "View toggles",
    ...
    dict(
        ...
        extra_measures=[
            (
                "DZV World",
                'SELECTEDVALUE(\'View data for\'[View data for], "World") = "World"',
                None,
                "View toggles",
            ...
            (
                "DZV Region",
                'SELECTEDVALUE(\'View data for\'[View data for], "World") = "Region"',
                None,
                "View toggles",
            ...
            (
                "DZV Country",
                'SELECTEDVALUE(\'View data for\'[View data for], "World") = "Country"',
                None,
                "View toggles",
...
def emit_table(spec):
    ...
    for m in spec["measures"]:
        out.append(emit_measure(t, *m))
        out.append("")
    for disp, src, dtype, hidden, summ in spec["cols"]:
        out.append(emit_column(t, disp, src, dtype, hidden, summ))
        out.append("")
    for nm, dax, dtype, hidden, summ, desc in spec["calc_cols"]:
        out.append(emit_column(t, nm, None, dtype, hidden, summ, calc_dax=dax, desc=desc))
        out.append("")
...
def emit_year_table():
    t = "Year"
    out = [
        f"/// Conformed Year dimension ({YEAR_MIN}-{YEAR_MAX}), M-generated. Deliberate composite-model enhancement: the faithful translation of a Tableau 'blend on Year' - single-direction 1:many to each year-bearing fact so a report can slice all sources by one Year.",
        f"table '{t}'",
        f"\tlineageTag: {lt(t, 'table')}",
        "",
    ]
...
def emit_param_table(p):
    ...
    # value measure
    out.append(
        f"\t/// Current slicer selection on '{t}'[{t}], defaulting to Tableau's current value when nothing is selected."
    )
    ...
    out.append("\t\tdisplayFolder: Parameter")
    out.append("")
    for nm, dax, fmt, folder, desc in p.get("extra_measures", []):
        out.append(emit_measure(t, nm, dax, fmt, folder, desc))
        out.append("")
    # column
...
with open(os.path.join(DEFN, "expressions.tmdl"), "w", encoding="utf-8") as f:
    f.write(
        f'expression DataFolder = "{esc_m(DATA_FOLDER)}" meta [IsParameterQuery=true, Type="Text", IsParameterQueryRequired=true]\n'
    )
